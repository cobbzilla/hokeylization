 ...
 * Сиз `es` жана `de` ичинде окшош каталог түзүмүнө жана файлдарга ээ `en`
 ...
 ### Жок
 Кээде сиздин `-m` өтө көп файлдарга дал келет. Ачык эле алып салуу үчүн `-e` / `--excludes` опциясын колдонуңуз
 ...
 *Индекстерди түзүүдө сиз бир гана киргизүү булагына ээ боло аласыз*
 ...
 `-I` / `--index` опциясын өткөрүңүз, маани файл болушу мүмкүн болгон индекс файлы түзүлө турган жерде.
 ...
 Эгер сиз өзүңүздүн шаблонуңузду көрсөтсөңүз, анда `-I` / `--index` үчүн файлды (каталог эмес) көрсөтүшүңүз керек.
 параметр
